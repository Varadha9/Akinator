...
async function seedDatabase() {
  try {
    ...
    const uri = process.env.MONGODB_URI || 'mongodb://localhost:27017/akinator';
    ...
    const questionMap = {};
    questions.forEach(q => {
      questionMap[q.text.toLowerCase()] = q._id;
    });
      ...
      if (char.profile) {
        Object.entries(char.profile).forEach(([attribute, value]) => {
          ...
          if (mapping) {
            ...
            if (questionId) {
              answers.push({ questionId, answer: value === 'yes' ? mapping.yesAnswer : mapping.noAnswer });
            }
          }
        });
      }
      return { name: char.name, category: char.category, answers };
    });
